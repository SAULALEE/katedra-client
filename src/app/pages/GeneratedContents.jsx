...
export default function GeneratedContents() {
  ...
  const { eventos, loading } = useHistorial();
        ...
        <main style={{ position:'relative', zIndex:5, flex:1, minWidth:0, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <header className="kt-main-pad" style={{ display:'flex', alignItems:'center', gap:'18px', padding:'26px 32px', borderBottom:'1px solid var(--kt-border-soft)' }}>
            <div style={{ minWidth:0 }}>
              <h1 className="kt-headtitle" style={{ fontFamily:"'Inter'", fontWeight:600, fontSize:'27px', lineHeight:1.15, letterSpacing:'-1.2px', color:'var(--kt-heading)', margin:0 }}>Historial de Contenidos</h1>
              <p style={{ fontFamily:"'Manrope'", fontWeight:500, fontSize:'13.5px', color:'var(--kt-muted)', margin:'3px 0 0' }}>Registro de auditoría de tus temarios: altas, ediciones, favoritos y eliminaciones. Los materiales generados se consultan desde cada temario.</p>
            </div>
          ...
          {/* Timeline View — read-only audit trail: no delete, no shortcut into the generated material */}
          <div className="kt-main-pad" style={{ flex:1, overflow:'auto', padding:'40px 32px 80px' }}>
            <div style={{ maxWidth:'900px', margin:'0 auto', position:'relative' }}>
              ...
              {loading ? (
                 ...
                 <div style={{ padding:'40px 60px', color:'var(--kt-faint)', fontFamily:"'Manrope'", fontSize:'14px' }}>Todavía no hay actividad registrada.</div>
              ) : (
                 eventos.map((evento) => {
                   ...
                   if (evento.tipo === 'CREADO') {
                     ...
                     ActionIcon = Settings;
                   } else if (evento.tipo === 'FAVORITO_AGREGADO') {
                     actionLabel = 'Agregado a Favoritos';
                     ActionIcon = Heart;
                     actionColor = '#F43F5E';
                     iconBg = 'rgba(244,63,94,.1)';
                     iconBorder = 'rgba(244,63,94,.2)';
                   } else if (evento.tipo === 'FAVORITO_QUITADO') {
                     actionLabel = 'Quitado de Favoritos';
                     ActionIcon = Heart;
                     ...
                     iconBg = 'rgba(244,63,94,.1)';
                     iconBorder = 'rgba(244,63,94,.2)';
                   } else if (evento.tipo === 'GENERADO') {
                     actionLabel = 'Contenido Generado';
                     ActionIcon = Sparkles;
                     actionColor = '#38BDF8';
                   ...
                   return (
                     <div key={evento.id} style={{ position:'relative', paddingLeft:'74px', marginBottom:'32px', display:'flex', flexDirection:'column', gap:'12px' }}>
                       ...
                       <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                         <span style={{ fontFamily:"'Manrope'", fontWeight:800, fontSize:'12px', letterSpacing:'1px', color:'var(--kt-muted)', textTransform:'uppercase' }}>{evento.createdAt ? new Date(evento.createdAt).toLocaleDateString('es-ES', { month:'long', day:'numeric', year:'numeric', hour:'2-digit', minute:'2-digit' }) : 'Reciente'}</span>
                       ...
                       <div className="kt-scard" style={{ display:'flex', flexDirection:'column', background:'var(--kt-card-bg)', border:'1px solid var(--kt-panel-border)', borderRadius:'16px', padding:'24px', backdropFilter:'blur(12px)' }}>
                         ...
                         <div style={{ display:'flex', gap:'16px', alignItems:'flex-start' }}>
                           ...
                           <div style={{ flex:1, minWidth:0 }}>
                             <h3 style={{ fontFamily:"'Inter'", fontWeight:600, fontSize:'20px', letterSpacing:'-.6px', color:'var(--kt-heading)', margin:'0 0 4px 0' }}>{evento.temarioTitulo}</h3>
                             <p style={{ fontFamily:"'Manrope'", fontWeight:500, fontSize:'14px', color:'var(--kt-muted)', margin:0 }}>{evento.asignaturaNombre}</p>
                           </div>
                         </div>

                         {evento.tipo === 'GENERADO' && evento.detalle && (
                           <div style={{ marginTop:'20px', paddingTop:'16px', borderTop:'1px solid var(--kt-border-soft)', display:'flex', alignItems:'center', gap:'20px' }}>
                             <span style={{ fontFamily:"'Manrope'", fontWeight:600, fontSize:'12px', color:'var(--kt-faint)' }}>Detalle:</span>
                             <span style={{ padding:'4px 10px', borderRadius:'8px', background:'rgba(56,189,248,.1)', fontFamily:"'Manrope'", fontWeight:700, fontSize:'11px', color:'#38BDF8' }}>{evento.detalle}</span>
                           </div>
                         )}
