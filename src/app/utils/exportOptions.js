 ...
 * should never be able to trigger a download that can only come back as a 404.
 *
 * @param {string} pieza backend piece name
 * @param {{ disponible?: boolean }} [opciones]
 */
export const opcionesDePieza = (pieza, { disponible = true } = {}) =>
  (OPCIONES_EXPORTACION[pieza] || []).map(opcion => ({ ...opcion, disabled: !disponible }));
