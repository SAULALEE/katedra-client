...
- **Exclusion:** Does not handle frontend state storage (Zustand) or frontend API service definitions (use `ARCHITECTURE.md`).
...
Output Expected:
```sql
-- 1. Flyway: src/main/resources/db/migration-postgresql/V26__create_module_table.sql
CREATE TABLE module (
